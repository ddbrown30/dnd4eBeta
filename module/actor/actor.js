...
export class Actor4e extends Actor {
	...
	prepareBaseData() {
		...
		for (let [id, abl] of Object.entries(system.abilities)) {
			...
			abl.modHalf = abl.mod + Math.floor(system.details.level);
			abl.prof = (abl.proficient || 0);
			if (game.settings.get("dnd4e", "halfLevelOptions")) {
				abl.saveBonus = saveBonus;
				abl.checkBonus = checkBonus;
			} else {
				abl.saveBonus = saveBonus + Math.floor(system.details.level);
				abl.checkBonus = checkBonus + Math.floor(system.details.level);
			}
	...
	prepareDerivedData() {
		...
		system.details.surgeValue = Math.floor(system.details.bloodied) + system.details.surgeBon.value;
		...
		if (!game.settings.get("dnd4e", "halfLevelOptions")) {
			initBonusValue += Math.floor(system.details.level);
		}
	...
	calcDefenceStatsCharacter(data) {
		for (let [id, def] of Object.entries(data.defences)) {
			...
			if (!game.settings.get("dnd4e", "halfLevelOptions")) {
				def.value += Math.floor(data.details.level);
			}
		}
	}
	...
	calcSkillCharacter(system) {
		for (const [id, skl] of Object.entries(system.skills)) {
			...
			if (!game.settings.get("dnd4e", "halfLevelOptions")) {
				skl.total += Math.floor(system.details.level);
			}
	...
	rollAbility(abilityId, options = {}) {
		...
		const data = game.settings.get("dnd4e", "halfLevelOptions") ? { mod: abl.mod } : { mod: abl.mod, halfLevel: Math.floor(this.system.details.level) };
